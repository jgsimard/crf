...
class HashTable(object):
    ...
    def filter_values(self):
        ...
        if unikeys.size(0) != self.indices.size(0):
            index = torch.sparse.LongTensor(reverse_t.unsqueeze(0), torch.ones(reverse_t.size(0)).cuda(),
                                            torch.Size([unikeys.size(0), ])).to_dense()
    ...
    def get_rank(self, keys):
        ...
        indices_sp_m = torch.sparse.LongTensor(indices, torch.ones(indices.size(1)).type(torch.FloatTensor).cuda(),
                                               torch.Size([indices.size(1), self.table_size]))
        index_rank_spm = torch.cat(
            [self.indices.unsqueeze(0), torch.zeros((1, self.indices.size(0))).type(torch.LongTensor).cuda()], 0)
        rank_sp_m = torch.sparse.LongTensor(index_rank_spm, self.rank.type(torch.cuda.FloatTensor),
                                            torch.Size([self.table_size, 1])).to_dense()
